                    ...
                    <div class="col-md-6 ml-5">
                        <form action="{{ route('settings.store') . '?type=' .request()->type }}" method="post">
                            ...
                            @elseif(request()->type === 'Textarea')
                                <div class="mb-3 mt-3">
                                    <label for="email" class="form-label">Config-value</label>
                                    <textarea rows="5" name="config_value" class="form-control @error('config_value') is-invalid @enderror" placeholder="Nhập Config-value:" ></textarea>
                                    @error('config_value')
                                    <div class="text text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <button type="submit" class="btn btn-primary">Thêm Setting</button>
                        </form>
                    </div>
